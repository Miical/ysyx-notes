# SUMMARY

* [简介](README.md)

* [预学习](预学习/1.如何科学的提问.md)

	* [1 如何科学的提问](预学习/1.如何科学的提问.md)
 
	* [2 Linux系统安装和基本使用](预学习/2.Linux系统安装和基本使用.md)
 
		* [The Missing Semester of Your CS Education](预学习/2a.The_Missing_Semester_of_Your_CS_Education.md)
  
	* [4 复习C语言](预学习/4.复习C语言.md)

	* [6 搭建verilator仿真环境](预学习/6.搭建verilator仿真环境.md)

	 * [7.数字电路基础实验](预学习/7.数字电路基础实验.md)

		* [Chisel Bootcamp](预学习/7a.Chisel%20Bootcamp/Chisel%20Bootcamp.md)
  
			* [1 Scala 介绍](预学习/7a.Chisel%20Bootcamp/1%20Scala%20介绍.md)

			* [2.1 第一个Chisel Module](预学习/7a.Chisel%20Bootcamp/2.1%20第一个Chisel%20Module.md)
   
			* [2.2 组合逻辑](预学习/7a.Chisel%20Bootcamp/2.2%20组合逻辑.md)

			* [2.3 控制流](预学习/7a.Chisel%20Bootcamp/2.3%20控制流.md)

			* [2.4 时序逻辑](预学习/7a.Chisel%20Bootcamp/2.4%20时序逻辑.md)

			* [2.5 综合：FIR 滤波器](预学习/7a.Chisel%20Bootcamp/2.5%20综合：FIR%20滤波器.md)

			* [2.6 Chisel Test](预学习/7a.Chisel%20Bootcamp/2.6%20Chisel%20Test.md)
   
			* [3.1 生成器参数](预学习/7a.Chisel%20Bootcamp/3.1%20生成器参数.md)
   
			* [3.2 生成器 Collections](预学习/7a.Chisel%20Bootcamp/3.2%20生成器%20Collections.md)
 
			* [3.3 高阶函数](预学习/7a.Chisel%20Bootcamp/3.3%20高阶函数.md)

			* [3.4 函数式编程](预学习/7a.Chisel%20Bootcamp/3.4%20函数式编程.md)
 
			* [3.5 面向对象编程](预学习/7a.Chisel%20Bootcamp/3.5%20面向对象编程.md)
   
		* [数字电路与计算机组成实验](预学习/7b.数字电路与计算机组成实验/7b.数字电路与计算机组成实验.md)
  
			* [实验一 选择器](预学习/7b.数字电路与计算机组成实验/实验一%20选择器.md)
   
			* [实验二 译码器和编码器](预学习/7b.数字电路与计算机组成实验/实验二%20译码器和编码器.md)
   
			* [实验三 加法器与ALU](预学习/7b.数字电路与计算机组成实验/实验三%20加法器与ALU.md)
   
			* [实验六 移位寄存器及桶型移位器](预学习/7b.数字电路与计算机组成实验/实验六%20移位寄存器及桶型移位器.md)

			* [实验七 状态机及键盘输入](预学习/7b.数字电路与计算机组成实验/实验七%20状态机及键盘输入.md)
   
	*  [8.完成PA1](预学习/8.完成PA1.md)
 
 * [基础阶段](基础阶段/基础阶段.md)
 
	 * [9.支持RV32IM的NEMU (PA 2.1)](基础阶段/9.支持RV32IM的NEMU%20(PA%202.1).md)
  
	 * [10 用RTL实现最简单的处理器](基础阶段/10%20用RTL实现最简单的处理器.md)
  
		 * [10.1 Scala](基础阶段/10.1%20Scala.md)
  
		 * [10.2 mill](基础阶段/10.2%20mill.md)
   
	 * [13 AM运行时环境](基础阶段/13%20AM运行时环境.md)